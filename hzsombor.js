...
rules: [/^(?:\s+)/,/^(?:IMÍGYEN SZÓLOTT VALA ZSOMBOR A HUNYADI\b)/,/^(?:MAJD HÁTAT FORDÍTA ÉS ELKÜLDÉ FEJLESZTŐIT A PICSÁBA\b)/,/^(?:[0-9]+)/,/^(?:FELKIÁLTA HÍVEI ELŐTT:)/,/^(?:SZÁMOLJÁTOK ÚGY HOGY\b)/,/^(?:ÉRTÉKE LÉGYEN\b)/,/^(?:ÁM EME ÉRTÉKRŐL MÁSKÉPP RENDELKEZEM\b)/,/^(?:TELJESEDJÉK BE\b)/,/^(?:MÁTÓL ISMERJÉTEK MÁSKÉNT AZT HOGY\b)/,/^(?:TEGYÉTEK HOZZÁ)/,/^(?:VEGYÉTEK EL\b)/,/^(?:SOKASÍTSÁTOK\b)/,/^(?:OSSZÁTOK EL TESTVÉRIESEN\b)/,/^(?:VIGYÉTEK EL NARADÉKÁT\b)/,/^(?:EGY TŐRŐL FAKAD\b)/,/^(?:NAGYOBB GYEREK-E MINT\b)/,/^(?:VAGY AZ IS JÓ HA\b)/,/^(?:ÉS VEDD MÉG FIGYELEMBE HOGY\b)/,/^(?:HA ÚGY LÁTJÁTOK HOGY\b)/,/^(?:DE HA MÁR ELBASZÓDOTT AKKOR\b)/,/^(?:VAGY CSINÁLJATOK AMIT AKARTOK BASZKI\b)/,/^(?:KEZDJÉTEK LEJTENI AZ ÖRÖK PIZZAFUTÁR TÁNCÁT AMÍG\b)/,/^(?:MAJD ÜLJETEK LE ÉS VÁRJÁTOK AZ ÉTKET\b)/,/^(?:A PRÓFÉTA PEDIG ELNEVEZÉ PIZZÁJÁT ÚGY MINT\b)/,/^(?:FELTÉTEI PEDIG LEGYENEK\b)/,/^(?:ÉS KÜLDJETEK HOZZÁ PIZZASZÓSZT VALA\b)/,/^(?:HA TETSZÉSETEK ELNYERI ÁLDÁSOMMAL ÉTLAPOTOKRA VEGYÉTEK\b)/,/^(?:DICSŐSÉG MERT ÍME MEGRENDELÉM A PIZZÁT MINEK NEVE\b)/,/^(?:[a-zA-Z]+)/,/^(?:"(?:[^"\\]|\\.)*")/,/^(?:$)/],
...
exports.parser = hzsombor;
exports.Parser = hzsombor.Parser;
exports.parse = function () { return hzsombor.parse.apply(hzsombor, arguments); };
